<template>
  <div class="text-center my-16">
    <h2 class="text-teal-400 text-2xl font-bold">THỂ LOẠI</h2>
    <h1 class="text-5xl font-bold mt-3 mb-8">{{ genresName }}</h1>
    <router-link
      to="/genres"
      class="bg-teal-400 drop-shadow-lg text-white py-2 px-6 rounded-full mt-4 hover:bg-teal-500 cursor-pointer"
    >
      Xem tất cả thể loại
    </router-link>
    <!-- <ButtonGreen value="Xem tất cả thể loại" /> -->
  </div>

  <div class="my-20" v-show="bookList.length == 0">
...
export default defineComponent({
  ...
  methods: {
    ...mapActions(["getAllBookByCategoryId"]),

    async setAllBook() {
      const id = this.$router.currentRoute.value.params.id;
      // console.log("id của genres: " + this.$route.params.id);
      await this.getAllBookByCategoryId(id);
      this.bookList = this.allBookbyCategoryGet || [];
    ...
    detailBook(idBook: number) {
      const idGenres = this.$router.currentRoute.value.params.id;
