      ...
      <div class="lg:flex lg:flex-row gap-10 px-5 lg:pl-10">
        <div id="cart" class="w-full flex-auto">
          <h1 class="my-8 text-4xl font-bold text-gray-700">
            ĐƠN HÀNG ĐANG XỬ LÝ ({{ setOrder ? setOrder.length : 0 }})
          </h1>
          <div class="my-5">
...
export default defineComponent({
  ...
  methods: {
    async testWaitingOrder(dataWait) {
      const bookListFilter = [];
      try {
        const { data, status } = await orderService.getWaitingOrder(1);
        if (status == 200) {
          console.log("Lấy tất cả đơn hàng có sẵn thành công");
          const dataAvailable = data.Data;
          console.log(dataAvailable);
          console.log(dataWait);
          if (dataAvailable) {
            dataWait.forEach((elementA) => {
              if (
                !dataAvailable.some((elementB) => elementB.Id === elementA.Id)
              ) {
                bookListFilter.push(elementA);
              }
            });
          ...
          //   console.log(dataWait);
          //   console.log(dataAvailable);
          //   console.log(bookListFilter);
        } else {
          console.log("Lấy tất cả đơn hàng có sẵn thất bại");
        }
      } catch (error) {
        console.log(error);
      }
      ...
      try {
        if (this.$cookies.isKey("token")) {
          ...
          if (status == 200) {
            ...
            this.order = await this.testWaitingOrder(data.Data);
            this.order.reverse();
